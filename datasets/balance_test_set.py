...
test_set=np.load('/content/drive/My Drive/cross/Cross-spectrum-EEG-master/test_set.npy',allow_pickle=True)
...
class TestSetBuilder:
  ...
  def __init__(self, size): #size->number of patients from which segment bank will be made
    ...
    self.size = size
    self.testset_dict = test_set.item()
  ...
  def create_testset(self):      #main
    ...
    for p in range(self.size): 
      ...
      print("\n")
      np.save(f"/content/drive/My Drive/cross/Cross-spectrum-EEG-master/test_set_balanced.npy", dataset.testset_dict)
...
dataset = TestSetBuilder(size=40)  #size->number of patients from which random segment  will be chosen  #patient 34 onwards have different channel name for EEG
