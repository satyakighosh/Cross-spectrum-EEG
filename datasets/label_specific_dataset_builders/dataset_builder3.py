...
class DatasetBuilder:
  ...
  def extract_random_segments_for_given_patient_during_warning(self,segment_label, patient_no):   #during warning related to AR(l)__autocorrelation lag
    ...
    return (int(segment_label),eeg_dict_[segment_label][(len(eeg_dict_[segment_label])-1)//2])
  ...
  def generate_features_with_ref_segments(self, selected_tuple,patient_no):
    ...
    for ref_segment in self.ref_segments[self.ref_label]:
      ...
      try:
        ...
        return self.generate_features_with_ref_segments(substitute,patient_no)
  ...
  def create_dataset_of_particular_stage(self, num_segs_chosen):      #main
    ...
    for p in range(self.size): 
      ...
      for segment in segment_generator:
        try:
          print(segment[0])
          segs.append(self.generate_features_with_ref_segments(segment,p))  #just appending the label for now, can save segment[1] for actual segment
        except Warning:
      ...
      print("\n")
      np.save(f"/content/drive/My Drive/cross/Cross-spectrum-EEG-master/clf{ref_label}.npy", self.data_list)
      print(f"Dataset saved after patient{p}")
...
np.save(f"/content/drive/My Drive/cross/Cross-spectrum-EEG-master/SOB.npy", dataset.data_list)
